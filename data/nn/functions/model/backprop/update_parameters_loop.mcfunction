...
execute if score .loop1 global matches 0 run function nn:model/backprop/update_bias with storage nn:helpers

# Increment inner loop
scoreboard players add .loop1 global 1

# If inner loop hasn't been exceeded, create next weight
$execute if score .loop1 global < .$(layer)_from global run return run function nn:model/backprop/update_parameters_loop with storage nn:helpers
